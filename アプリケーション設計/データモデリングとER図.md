...
## 第二正規化
+ 値(行)の重複を無くす
+ ECサイトの例なら、ある注文商品Aと注文商品Bでは、同じタイミングで注文したために、「注文日」や「支払い方法」が重複するケース
+ この状態をエクセルの表でイメージすると、複数行のある列の値が重複している状態
+ この重複した行を、一つの独立した情報にすることを通じて、重複をなくすことを第二正規化という
+ ただし、複数行で値が重なるケースはいくらでもあり、その分だけリソースを増やすと肥大化しないだろうか？
